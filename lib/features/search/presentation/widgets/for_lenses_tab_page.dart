...
class _ForGlassesTabPageState extends State<ForLensesTabPage> {
  ...
  _buildLensesColorWidget(
      ...
      List<ExtraGlassesItemEntity> listOfColor = const []}) {
    return Padding(
  ...
  _buildLensCompanyTitleWidget(
      {required BuildContext context, required double width, double? height}) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(
        EdgeMargin.small,
        EdgeMargin.min,
  ...
  radioButtonItem({String? title, bool? isActive = false, Function()? onTap}) {
    return InkWell(
      onTap: onTap,
      child: Container(
        child: Row(
